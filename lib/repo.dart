...
List<Repo> reposFromJson(String str) => List<Repo>.from(json.decode(str).map((x) => Repo.fromJson(x)));

String reposToJson(List<Repo> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
...
enum EventsUrl { HTTPS_API_GITHUB_COM_USERS_F4_TIH35_EVENTS_PRIVACY }

final eventsUrlValues = EnumValues({
  "https://api.github.com/users/f4tih35/events{/privacy}": EventsUrl.HTTPS_API_GITHUB_COM_USERS_F4_TIH35_EVENTS_PRIVACY
});

enum FollowingUrl { HTTPS_API_GITHUB_COM_USERS_F4_TIH35_FOLLOWING_OTHER_USER }

final followingUrlValues = EnumValues({
  "https://api.github.com/users/f4tih35/following{/other_user}": FollowingUrl.HTTPS_API_GITHUB_COM_USERS_F4_TIH35_FOLLOWING_OTHER_USER
});

enum GistsUrl { HTTPS_API_GITHUB_COM_USERS_F4_TIH35_GISTS_GIST_ID }

final gistsUrlValues = EnumValues({
  "https://api.github.com/users/f4tih35/gists{/gist_id}": GistsUrl.HTTPS_API_GITHUB_COM_USERS_F4_TIH35_GISTS_GIST_ID
});

enum Login { F4_TIH35 }

final loginValues = EnumValues({
  "f4tih35": Login.F4_TIH35
});

enum NodeId { MDQ6_VX_NLCJ_UY_MD_AW_MJ_Q5 }

final nodeIdValues = EnumValues({
  "MDQ6VXNlcjUyMDAwMjQ5": NodeId.MDQ6_VX_NLCJ_UY_MD_AW_MJ_Q5
});

enum StarredUrl { HTTPS_API_GITHUB_COM_USERS_F4_TIH35_STARRED_OWNER_REPO }

final starredUrlValues = EnumValues({
  "https://api.github.com/users/f4tih35/starred{/owner}{/repo}": StarredUrl.HTTPS_API_GITHUB_COM_USERS_F4_TIH35_STARRED_OWNER_REPO
});
